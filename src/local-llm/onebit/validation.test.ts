...
describe('onebit validation helpers', () => {
  it('inspects onebit metadata from a converted GGUF blob', async () => {
    const { buffer } = buildToyModelGGUF({
      weightType: GGMLType.F32,
      outFeatures: 8,
      inFeatures: 16,
    });
    const converted = convertToOnebit(buffer);
    const blob = new Blob([converted.data], { type: 'application/octet-stream' });

    const summary = await inspectOnebitMetadata(blob);

    expect(summary.hasOnebitVersion).toBe(true);
    expect(summary.onebitVersion).toBe(1);
    expect(summary.signPacking).toBe('msb_first');
    expect(summary.onebitTensorCount).toBeGreaterThan(0);
  });
