/**
 * tensorFilter.ts — Tensor selection modes and classification for onebit conversion.
 *
 * Allows restricting which weight tensors are onebit-converted,
...
export interface OnebitConvertModeInfo {
  id: OnebitConvertMode;
  label: string;
  description: string;
}
...
export interface TensorConvertRecord {
  ...
  family: string;
  /** Whether this tensor was onebit-converted */
  converted: boolean;
  /** NMSE if converted and quality was computed */
  nmse: number | null;
  /** Original tensor size in bytes */
  originalSizeBytes: number;
  /** Onebit triplet size in bytes (a + b + sign), null if not converted */
...
export const ONEBIT_CONVERT_MODES: OnebitConvertModeInfo[] = [
...
const MODE_FAMILIES: Record<OnebitConvertMode, string[] | 'all'> = {
  'all': 'all',
