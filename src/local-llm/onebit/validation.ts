...
import { generateOnebitModelId, generateOnebitFilename } from './onebitManager';
import type { OutputQualityMetrics } from './qualityMetrics';

export type ValidationVariant = 'original' | 'onebit';
...
export interface OnebitMetadataSummary {
  hasOnebitVersion: boolean;
  onebitVersion: number | null;
  signPacking: string | null;
  layers: number[];
  tensorCount: number;
  onebitTensorCount: number;
}
...
export const VALIDATION_RUNS_STORAGE_KEY = 'onebit-validation-runs-v1';

export const FIXED_VALIDATION_MODEL = {
  id: 'onebit-validation-tinyllama-1.1b-q8',
...
export function getValidationModelDefinition(variant: ValidationVariant): LocalModelDefinition {
  ...
  const onebitFileName = generateOnebitFilename(FIXED_VALIDATION_MODEL.fileName);
  return {
    id: generateOnebitModelId(FIXED_VALIDATION_MODEL.id),
    engine: 'wllama',
    tasks: ['generation', 'analysis'],
...
export function createInitialValidationSteps(): ValidationStepState[] {
  return [
    ...
    { key: 'convert-onebit', label: 'onebit変換', status: 'idle' },
    { key: 'save-opfs', label: 'OPFS保存', status: 'idle' },
    { key: 'detect-onebit-metadata', label: 'onebit metadata 検出', status: 'idle' },
    { key: 'load-generate-original', label: '原本 load/generate', status: 'idle' },
    { key: 'load-generate-onebit', label: 'onebit load/generate', status: 'idle' },
...
export async function inspectOnebitMetadata(file: Blob): Promise<OnebitMetadataSummary> {
  const header = await parseHeaderFromBlob(file);
  const versionEntry = header.metadata.get(ONEBIT_VERSION_KEY);
  const signPackingEntry = header.metadata.get(ONEBIT_PACKING_KEY);
  const layersEntry = header.metadata.get(ONEBIT_LAYERS_KEY);
  const onebitTensorCount = header.tensors.filter((tensor) =>
  ...
  return {
    hasOnebitVersion: typeof versionEntry?.value === 'number',
    onebitVersion: typeof versionEntry?.value === 'number' ? versionEntry.value : null,
...
export function summarizeOutputComparison(originalOutput: string, onebitOutput: string): OutputComparisonSummary {
  return {
    exactMatch: originalOutput === onebitOutput,
    originalLength: originalOutput.length,
    onebitLength: onebitOutput.length,
    lengthDelta: onebitOutput.length - originalOutput.length,
  };
}
