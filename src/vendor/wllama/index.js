...
var ProxyToWorker = class {
  ...
  /**
   * Initialize the Emscripten module and wait for cwrap-ready, then terminate
   * the worker. Used by preflightInit() — no model files are needed.
   */
  moduleInitPreflightOnly() {
    return __async(this, null, function* () {
      if (!this.pathConfig["wllama.wasm"]) {
        throw new Error('"single-thread/wllama.wasm" is missing from pathConfig');
      }
      ...
      // wasmRuntimeInitialized is now true in the outer worker (inner-cwrap-ready
      // was posted before module.init resolved). Terminate — no model needed.
      this.worker.terminate();
      this.worker = void 0;
    });
  }
  /**
   * Initialize the worker for OPFS-direct loading.
   * Starts the Emscripten module and opens SyncAccessHandles for each shard.
   * @param modelId OPFS directory name under models/
   * @param shards Array of shard filenames (used as both memfsName and opfsFilename)
   */
  moduleInitOpfsDirect(modelId, shards) {
    return __async(this, null, function* () {
      if (!this.pathConfig["wllama.wasm"]) {
        throw new Error('"single-thread/wllama.wasm" is missing from pathConfig');
      }
      ...
      const shardDescriptors = shards.map((filename) => ({
        memfsName: filename,
        opfsFilename: filename
      }));
      yield this.pushTask({
        verb: "fs.opfs-setup",
        args: [modelId, shardDescriptors],
        callbackId: this.taskId++
      });
    });
  }
...
var Wllama = class {
  ...
  /**
   * Initialize the Emscripten module without loading a model.
   * Waits for the wasm runtime to be fully ready (inner-cwrap-ready), then
   * terminates the worker. Used by the preflight check to verify the WASM
   * glue loads correctly without requiring any model files.
   */
  preflightInit(_0) {
    return __async(this, arguments, function* (config = {}) {
      ...
      try {
        yield this.proxy.moduleInitPreflightOnly();
      } finally {
        this.proxy = void 0;
      }
    });
  ...
  /**
   * Load a model from OPFS using SyncAccessHandles (no heap copy).
   * @param modelId OPFS directory name under models/
   * @param shards Array of shard filenames
   * @param config LoadModelConfig
   */
  loadModelFromOpfs(_0, _1) {
    return __async(this, arguments, function* (modelId, shards, config = {}) {
      var _a, _b, _c;
      if (this.proxy) {
        throw new WllamaError("Module is already initialized", "load_error");
      }
      ...
      try {
        yield this.proxy.moduleInitOpfsDirect(modelId, shards);
        const startResult = yield this.proxy.wllamaStart();
        if (!startResult.success) {
          throw new WllamaError(
            `Error while calling start function, result = ${startResult}`
        ...
        const loadResult = yield this.proxy.wllamaAction("load", {
          _name: "load_req",
          use_mmap: false,
          // OPFS SyncAccessHandles don't support mmap
          use_mlock: true,
          ...
          n_threads: this.useMultiThread ? nbThreads : 1,
          n_ctx_auto: false,
          model_paths: shards.map((s) => `models/${s}`),
          embeddings: config.embeddings,
          offload_kqv: config.offload_kqv,
          n_batch: config.n_batch,
          pooling_type: config.pooling_type,
        ...
        yield this.proxy.opfsCleanup().catch(() => {
        });
        this.proxy = void 0;
        throw err;
      }
