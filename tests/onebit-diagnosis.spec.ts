/**
 * Playwright E2E tests for onebit quality diagnosis features.
 *
 ...
 *
 * Run all:
 *   npx playwright test tests/onebit-diagnosis.spec.ts --headed
 *
 * Run one step:
 *   npx playwright test tests/onebit-diagnosis.spec.ts -g "Step 1" --headed
 */
...
const BASE_URL = 'http://localhost:5175/?onebit-validation=1';
...
test.describe.serial('Onebit Quality Diagnosis', () => {
  ...
  test('Step 1: UI, import, convert, tensor metrics', async () => {
    ...
    await expect(page.locator('text=wllama onebit 品質診断UI')).toBeVisible();
    ...
    // --- Convert ---
    console.log('\n========== Convert to onebit (all) ==========');
    await clickActionButton(page, 'onebit変換');

    const convertResult = await waitForStepStatus(page, 'onebit変換', CONVERSION_TIMEOUT);
    ...
    const metadataResult = await waitForStepStatus(page, 'onebit metadata 検出', 60_000);
    ...
    await page.screenshot({ path: 'tests/onebit-step1-result.png', fullPage: true });
  ...
  test('Step 2: original model 9 prompts', async () => {
    ...
    fs.writeFileSync(
      path.join(path.resolve('tests'), 'onebit-step2-original.json'),
      JSON.stringify(results, null, 2),
    );

    await page.screenshot({ path: 'tests/onebit-step2-result.png', fullPage: true });
    console.log('Step 2 PASS');
  });

  // =========================================================================
  // Step 3: Onebit model — 9 prompts, one by one
  // =========================================================================
  test('Step 3: onebit model 9 prompts', async () => {
    ...
    for (let i = 0; i < promptOptions.length; i++) {
      const label = promptOptions[i];
      console.log(`\nOnebit ${i + 1}/${promptOptions.length}: ${label}`);

      await promptSelect.selectOption({ index: i });
      await page.waitForTimeout(200);
      await clickActionButton(page, 'onebitを実行');

      const result = await waitForStepStatus(page, 'onebit load/generate', 15 * 60_000);
      console.log(`  ${result.status} - ${result.detail}`);

      const outputPre = page.locator('.text-slate-600:has-text("onebit")').locator('..').locator('pre');
      const output = await outputPre.textContent().catch(() => '');
      const chars = output?.length ?? 0;
      console.log(`  ${chars} chars`);
    ...
    fs.writeFileSync(
      path.join(path.resolve('tests'), 'onebit-step3-onebit.json'),
      JSON.stringify(results, null, 2),
    );

    await page.screenshot({ path: 'tests/onebit-step3-result.png', fullPage: true });
  ...
  test('Step 4: export and comparison', async () => {
    ...
    const savePath = path.join(path.resolve('tests'), 'onebit-diagnosis-export.json');
    ...
    if (Array.isArray(exportJson)) {
      ...
      const onebitRuns = exportJson.filter((r: Record<string, unknown>) => r.variant === 'onebit');
      console.log(`Original: ${originalRuns.length} runs, Onebit: ${onebitRuns.length} runs`);

      console.log('\n--- Quality Comparison ---');
      console.log('Prompt                   | Orig chars | 1bit chars | Delta');
      console.log('-------------------------|------------|------------|------');
      for (const onebit of onebitRuns) {
        const orig = originalRuns.find((r: Record<string, unknown>) => r.promptId === onebit.promptId);
        const origLen = typeof orig?.output === 'string' ? orig.output.length : 0;
        const onebitLen = typeof onebit.output === 'string' ? onebit.output.length : 0;
        const pid = String(onebit.promptId ?? '').padEnd(24);
        console.log(`${pid} | ${String(origLen).padStart(10)} | ${String(onebitLen).padStart(10)} | ${onebitLen - origLen}`);
      }
    }

    await page.screenshot({ path: 'tests/onebit-step4-result.png', fullPage: true });
    console.log('\nStep 4 PASS');
  });
});
